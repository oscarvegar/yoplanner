...
module.exports = {
	findByRFP: function(req, res) {
		...
		RFP.findOne({
			id: params.rfp
		}).populate('recintos').exec(function(err, data) {
		...
		Recinto.findOne().where({
			...
			if (!data) {
				...
				Recinto.find().where({
					...
					page: page,
					limit: pagesize
				}).populate('comentarios').sort("place DESC").sort("starRating DESC").then(function(data) {
					return res.json(data);
				})
			}
		});
	...
	getBackgroundDestino: function(req, res) {
		...
		Destino.findOne({
			...
			console.log(data);
			return res.json({
				foto: data.fotoPrincipal ? data.fotoPrincipal : 'img/yp-backgrounds/ACA/Hoteles en Acapulco.jpg'
			});
		}).catch(function(err) {
			console.log(err);
			return res.json({
				foto: 'img/yp-backgrounds/ACA/Hoteles en Acapulco.jpg'
			});
		});
	},
	...
	findByIdAndCiudad: function(req, res) {
		...
		Recinto.findOne({
			...
			if (data.cityId == idciudad) {
				...
				return res.json({
					error: true,
					message: 'Hotel no es del destino actual.'
				});
			}
		});
	},
	...
	serveIFrame: function(req, res) {
		...
		if (tipo == 'list') {
			...
			Recinto.findOne({
				id: id
			}).populateAll().then(function(hotel) {
			...
			Destino.findOne({
				id: id
			}).populateAll().then(function(data) {
