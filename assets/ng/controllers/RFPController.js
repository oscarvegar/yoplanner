angular.module('rfp-module', [])
	...
	.controller('RFPController', function($scope, $http, $rootScope, $timeout, Upload, notify) {
      ...
      //Cargar email hoteles
      $rootScope.hotelesSeleccionados.forEach(function (hotel) {
        $http.get("/recinto/findById/" + hotel.id).then(function(hotelRes) {
		...
		$scope.muestraCuadritos = function() {
			...
			if ($scope.fechaInicialFor && !$scope.fechaFinalFor) {
				...
				for (var i = 0; i < totalDias; i++) {
					$scope.configuracionHabitacionesSencillas.push({
						tipoHabitacion: 1,
						fecha: moment($scope.rfp.fechaInicial).add(i, 'day').toDate()
					});
					$scope.configuracionHabitacionesDobles.push({
						tipoHabitacion: 2,
						fecha: moment($scope.rfp.fechaInicial).add(i, 'day').toDate()
					});
		...
		$scope.agregarEventoSalon = function() {
			...
			$timeout(function() {
				$scope.refrescarSalones($scope.rfp.salones.length - 1)
			}, 300);
		}
		...
		$scope.regresarDatos = function() {
			if ($scope.rfp.needSal) $.fancybox.prev();
			else {
				$.fancybox.prev();
				$.fancybox.prev();
			}
